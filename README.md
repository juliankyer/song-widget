...
We've been mentioning "state" and "store" a lot. The store is a big object that holds our state, and we only get one. We use ```createStore()``` to build this object. We are passing ```createStore()``` the rootReducer, the Chrome devTools extension, and an enhancer (which is a function). The enhancer we're using is ```applyMiddleware()```, which comes out of the box with Redux. This let's logger and thunk interact with the store (logger console logs state changes and actions; it might be overkill if you're already using the Chrome devTools extension, but it can nicely illustrate how store responds to async events).
